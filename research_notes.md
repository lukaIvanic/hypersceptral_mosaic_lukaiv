...
| 14 | Native 1024 UNet-lite + residual head + spectral conv (k=5), direct 61ch | unet-lite-res1024-v5-reshead-spec-conv-prof | resize=1024, bs=2; `model_variant=unet_lite`; `unet_base_channels=64`; `latent_channels=32`; `encoder_depth=6`; `coarse_channels=61` (no spectral interpolation); lr=1e-3 cosine (warmup 5, η_min=1e-5); λ `{L1:1.0, SAM:0.2, SID:0.0}`; `--use-residual-head --use-spectral-conv --spectral-conv-kernel-size 5`; epochs=180; profiling flags enabled | **7.4429** | **0.0286** | **10.8938** | **41.0711** | **0.9704** | **1.5087** | Best ep 177 (loss **0.0062**). Kaggle accuracy **0.3331**. Improves over Stage 13 (+0.8 pp); spectra smoother with 1D conv and direct 61‑channel head; band‑edge MAE remains the highest. | completed |
| … | (extend as needed) | | | | | | | | | | |
